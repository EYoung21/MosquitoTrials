#!/bin/bash
#SBATCH --job-name=epg_mq_rf_binary_optuna
...
uv run --extra cu129 model_eval_binary_np_p.py \
  --data_path /data/labs/hopelab/epg/tarsalis_data_clean \
  --save_path "$MOSQUITO_DIR/eliIntroLabsResults/tarsalis_results_smote_binary_optuna" \
  --model_path rf_sunwindow_smote.py \
  --model_name rf_sunwindow_smote_binary \
  --optuna \
  --augment
