...
class Model():
    ...
    def train(self, probes, test_probes, **kwargs):
        ...
        if loss_gamma != 0. or loss_alpha is not None:
            criterion = FocalLoss(alpha=loss_alpha, gamma=loss_gamma, reduction='mean')
    ...
    def predict_proba(self, probes, batch_size, preprocess = False):
        test_dataset = TimeSeriesDataset(probes, self.label_map, data_columns=self.data_columns,
                                         class_column = "labels", ignore_N=self.ignore_N)
        test_dataloader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False)
        all_probabilities = []
