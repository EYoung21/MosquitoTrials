...
SAVE_PATH = SHARPSHOOTER_DIR / "sharpshooter_rf_samchan_eval"
MODEL_NAME = "rf_samchan_best_t71"
...
def generate_report_mosquito_style(
    ...
    # difference plots (one png per probe/segment), saved into a subfolder
    diff_dir = save_path / "difference_plots"
    ...
    for df, preds, name in zip(test_data, predicted_labels, test_names):
        ...
        fig.savefig(diff_dir / f"{model_name}_{stem}_Fold{fold}.png")
        plt.close(fig)
...
def build_best_samchan_model(save_path: str | None = None) -> SamchanModel:
    """
    Instantiate rf_samchan.Model with the best hyperparameters from the
    rf_samchan3 Optuna run (Trial 71: F1 ≈ 0.7513).
    """
    model = SamchanModel(save_path=save_path)

    # Best params from rf_samchan3 (Trial 71)
    model.chunk_seconds = 13
    model.num_freqs = 10
    model.num_estimators = 256
    model.max_depth = 16
    model.max_features = 0.34586961015293977
    model.overlap = 0.6521889678903741
    model.max_lag = 19
...
def main() -> None:
    ...
    print("Loading sharpshooter data for rf_samchan3 best Trial 71 evaluation...")
    ...
    for fold, (train_index, test_index) in enumerate(data.cross_val_iter):
        print(f"=== Evaluating rf_samchan3 best Trial 71 hyperparams, fold {fold} ===")
    ...
    print("rf_samchan3 Trial 71 evaluation complete.")
