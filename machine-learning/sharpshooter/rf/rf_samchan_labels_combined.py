...
class Model:
    ...
    def transform_data(self, probes, training=True):
        ...
        for probe in probes:
            n_samples = len(probe)
            # Never skip short probes: evaluate one padded window instead.
            # This avoids biasing metrics against short/fragmented events.
            if n_samples == 0:
                if not training:
            ...
            for start in start_indices:
                ...
                else:  # pad short/last window with last value
    ...
    def predict(self, probes):
        ...
        for df, raw_probe in zip(transformed_probes, probes):
            ...
            # last window (safe for short probes where n_samples < chunk_size)
            last_start = max(0, n_samples - self.chunk_size)
            last_end = n_samples
            last_len = last_end - last_start
            if last_len > 0:
                last_sl = slice(last_start, last_end)
                # Align to the right edge of the window for truncated short probes.
