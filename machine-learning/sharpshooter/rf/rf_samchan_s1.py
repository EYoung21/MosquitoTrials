...
class Model:
    def __init__(self, save_path=None, trial=None):
        ...
        # s1: historical rf_samchan-style bounds (pre directional expansion)
        if trial:
            self.chunk_seconds = trial.suggest_int('chunk_seconds', 1, 20)
            ...
            self.max_features = trial.suggest_float('max_features', 0.3, 1.0)
            self.chunk_size = self.chunk_seconds * self.sample_rate
            self.overlap = trial.suggest_float('overlap', 0.25, 0.8)
            self.max_lag = trial.suggest_int('max_lag', 3, 20)
    ...
    def transform_data(self, probes, training=True):
        ...
        for probe in probes:
            ...
            if n_samples < self.chunk_size:
                print(f"Skipping probe: too short (len={n_samples}, chunk_size={self.chunk_size})")
                if not training:
            ...
            for start in start_indices:
                ...
                columns["spec_bandwidth"].append(
                    np.sqrt(np.sum(((freqs[:len(fft_vals)] - columns["spec_centroid"][-1]) ** 2) * spectral_power))
                )
            ...
            _current = (0 if probe["current"].values[0] == "AC" else 1) if "current" in probe.columns else 0
            columns["current"] = [_current] * n_windows
